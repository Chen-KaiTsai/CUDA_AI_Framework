...
__global__ void ReLU(float* X, float* Y)
{
...
framework::relu::relu(layer* p, bool inplace_, dim3 blockSize)
{
    name = "ReLU";
    prvLayer = p;
...
void framework::relu::run()
{
    ...
    cudnnSetActivationDescriptor(relu_desc, mode, prop, 0.0f);

    float alpha[1] = {1.0f};
    float beta[1] = {0.0f};

    float* prvMEM = prvLayer->getGPUMem();
    cudnnActivationForward(handle, relu_desc, alpha, prvMEM_desc, prvMEM, beta, gMEM_desc, gMEM);
...
void framework::relu::run()
{
	...
	ReLU<<<dimGrid, dimBlock>>>(prvMEM, gMEM);
	cudaDeviceSynchronize();
...
size_t framework::relu::setParam(float* buffer) { return 0; }
